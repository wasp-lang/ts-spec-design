...
// EDIT: Miho pointed out that for now, author of full stack module can just configure, in the entry-point / config / init function of the full stack module, what the module itself returns -> which pages, routes, ... . So they can configure it that way, based on user input, instead of us doing it in the "smart way" as I was proposing above. Which then sounds like the best solution for now, as it is simple and works, and we can easily later upgrade on top of it when we know more.
